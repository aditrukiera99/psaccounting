...
class Rekonsiliasi_bank_c extends CI_Controller {
	...
	public function index()
	{
		...
		$data_proyek = $this->db->query("SELECT * FROM ak_proyek ORDER BY ID DESC")->result();
		$data_departemen = $this->db->query("SELECT * FROM ak_departemen ORDER BY ID DESC")->result();
		$data_akun_kas = $this->db->query("SELECT * FROM ak_kode_akuntansi WHERE KATEGORI = 'Kas & Bank' AND ANAK_DARI != '' ORDER BY ID DESC")->result();
		...
		$data = array(
			'page' => 'rekonsiliasi_bank_v',
			'data_item' => $data_item, 
			'data_departemen' => $data_departemen, 
			'data_akun_kas' => $data_akun_kas, 
			'data_akun' => $data_akun, 
